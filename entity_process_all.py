...
def process_gene(entity_type, id_type, file_path, selected_column, feature_label):
    ...
    def process_gene_expression(gene_entity_data):
        ...
        gene_expression_data = merged_data[cols]
        ...
        gene_expression_data = pd.merge(gene_expression_data, gene_entity_data[['MedGraphica_ID', 'HGNC_Symbol']], on='MedGraphica_ID', how='left')

        # Rename 'HGNC_Symbol' to 'Gene_Name'
        gene_expression_data.rename(columns={'HGNC_Symbol': 'Gene_Name'}, inplace=True)
    ...
    def process_gene_cnv(gene_entity_data):
        ...
        cnv_aggregated['Gene_Name'] = cnv_aggregated['Gene_Name'].apply(lambda x: x.split('-')[0] if pd.notnull(x) else x)
    ...
    def process_gene_methyl(gene_entity_data):
        ...
        # Create a cross-product of all MedGraphica_ID, Gene_Name, and Region to ensure every combination is present
        all_combinations = (
            methylation_merged[['MedGraphica_ID', 'Gene_Name']].drop_duplicates()
            .assign(key=1)  # Temporary key to create Cartesian product
            .merge(pd.DataFrame(unique_regions, columns=['Region']).assign(key=1), on='key')
            .drop(columns=['key'])
        )

        # Merge all_combinations with the original methylation_merged data, and fill missing values with 0
        completed_methylation = pd.merge(
            all_combinations,
            methylation_merged,
            on=['MedGraphica_ID', 'Gene_Name', 'Region'],
            how='left'
        ).fillna(0)

        # Print the shape of the completed methylation data
        print(f"Shape of completed methylation data: {completed_methylation.shape}")

        # Iterate over unique regions and export each region-specific DataFrame
        for region in unique_regions:
            # Create a DataFrame specific to the current region
            region_df = completed_methylation[completed_methylation['Region'] == region]
            print(f"Shape of region {region}_df: {region_df.shape}")
            ...
            print(f"Exported merged {region} data to {output_file_path}")
